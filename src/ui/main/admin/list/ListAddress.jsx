...
import Address from '../../../../components/list/address';
...
const ListAddress = () => {
    ...
    const getData = async () => {
        try {
            setData([]);
            const dataAddress = await AxiosInstance().get('/location/getAll');
            if (dataAddress.data && dataAddress.data.length > 0) {
                setData(dataAddress.data);
                const dataFiltered = dataAddress.data.filter(item => item.isActive === true);
        ...
        <div className='drak:p-4 dark:bg-gray-900 w-1/3'>
            <div className="p-4 relative overflow-x-auto shadow-md sm:rounded-lg">
                <div className="pb-4 flex justify-between item-center">
                    ...
                    <div className='text-gray-900 dark:text-white text-2xl'>Nơi đặt trụ sạc</div>
                ...
                <Address data={dataFil} number={number} isPaused={isPaused} search={setSearchKey} />
