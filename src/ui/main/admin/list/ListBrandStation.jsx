...
import BrandStation from '../../../../components/list/brandstation';
...
const ListBrandStation = () => {
    ...
    const getData = async () => {
        try {
            setData([]);
            const dataBrandStation = await AxiosInstance().get('/brand/getAll');
            if (dataBrandStation.data && dataBrandStation.data.length > 0) {
                setData(dataBrandStation.data);
                const dataFiltered = dataBrandStation.data.filter(item => item.isActive === true);
        ...
        <div className='drak:p-4 dark:bg-gray-900 w-1/3'>
            <div className="p-4 relative overflow-x-auto shadow-md sm:rounded-lg">
                <div className="pb-4 flex justify-between item-center">
                    ...
                    <div className='text-gray-900 dark:text-white text-2xl'>Hãng trạm sạc</div>
                ...
                <BrandStation data={dataFil} number={number} isPaused={isPaused} search={setSearchKey} />
